...
Section WithArgs.
  ...
  (* read is the same as live, but without ever
     removing variables *)
   Fixpoint read(s: stmt var): list var :=
    match s with
    | SSet x y
    | SLoad _ x y _
    | SInlinetable _ x _ y => [y]
    | SStore _ a x _ => [a; x]
    | SStackalloc x _ body => read body
    | SLit x _ => []
    | SOp x _ y z => let var_z := match z with
                                | Var vz => [vz]
                                | Const _ => []
                     ...
                     ([y] ++ var_z)
    | SIf c s1 s2 => list_union String.eqb
                       (list_union String.eqb (read s1) (read s2))
                       (accessed_vars_bcond c)
    | SSeq s1 s2 => list_union String.eqb (read s1) (read s2)
    | SLoop s1 c s2 =>
        list_union String.eqb (read s1)
          (list_union String.eqb (accessed_vars_bcond c)
             (read s2))
    | SSkip => []
    | SCall binds _ args
    | SInteract binds _ args => args
    end.

   (* live' is the same as live, but
      with more conservative guesses for SLoop,
      to match DeadAssignment *)
  ...
  Fixpoint deadAssignment(used_after: list var)(s: stmt var) : stmt var :=
    ...
    | SLoop s1 c s2 => let read_vars := read s in
                       let used_after' := list_union String.eqb
                                            read_vars
                                            used_after
                       in SLoop (deadAssignment used_after' s1) c (deadAssignment used_after' s2)
